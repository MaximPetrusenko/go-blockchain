...
func NewWallet() *Wallet {
	...
	// 5. Perform SHA-256 hash on the extendend RIPEMD-160 result.
	h5 := sha256.New()
	h5.Write(vd4)
	digest5 := h5.Sum(nil)

	// 6. Perform SHA-256 hash on the result of the previoud SHA-256 hash.
	h6 := sha256.New()
	h6.Write(digest5)
	digest6 := h6.Sum(nil)
...
func (w *Wallet) BlockchainAddresss() string {
	return w.blockchainAddresss
}
